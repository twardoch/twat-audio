...
def normalize_audio(
    input_path: str | Path, output_path: str | Path, *, target_i: float = -16.0, dry_run: bool = False
) -> CommandResult:
    """Normalize loudness with ffmpeg's loudnorm filter."""
    return run_ffmpeg(
        ["-y", "-i", str(input_path), "-af", f"loudnorm=I={target_i:g}:TP=-1.5:LRA=11", str(output_path)],
        dry_run=dry_run,
    )
...
def extract_audio(
    input_path: str | Path, output_path: str | Path, *, codec: str = "pcm_s16le", dry_run: bool = False
) -> CommandResult:
...
def replace_audio(
    video_path: str | Path, audio_path: str | Path, output_path: str | Path, *, dry_run: bool = False
) -> CommandResult:
...
def simple_effect(
    input_path: str | Path, output_path: str | Path, effect: str, *, dry_run: bool = False
) -> CommandResult:
